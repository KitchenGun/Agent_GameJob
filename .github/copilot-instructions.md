# Agent GameJob — GitHub Copilot Instructions
# (Canonical source: CLAUDE.md — keep in sync when updating)

## 프로젝트 개요
게임 프로그래머 채용공고 자동 수집 → Google Sheets 저장 → Hermes Agent 위임 → Discord 알림 파이프라인.
Python 3.11+, Playwright + BeautifulSoup4 크롤링, gspread Google Sheets 연동.
...
6. Hermes Agent(별도 프로세스)가 파일을 읽어 LLM 판단 후 Discord 전송
...
| `config.py` | 환경변수 래퍼 |
| `crawlers/base_crawler.py` | 크롤러 기반 클래스 — 새 크롤러는 반드시 상속 |
| `sheets/sheets_manager.py` | Google Sheets 읽기/쓰기/상태변경, `cleanup_closed_jobs` |
| `agents/job_management_agent.py` | Hermes 파일큐 요청 JSON 생성 |
| `match_discord.py` | Discord용 매칭 데이터 추출 스크립트 |

## 설계 제약 — 코드 제안 시 반드시 지켜야 할 사항

- **Hermes Agent가 매칭·Discord 전송을 담당한다.** `matcher/` 레거시 점수식을 재활성화하거나 Python 코드에서 Discord 웹훅을 직접 호출하지 말 것.
- **공고 필터링은 `_is_programmer_job()`으로 통일.** 새 크롤러를 추가할 때도 이 함수를 거친다.
- **중복 제거 키는 `_job_signature(url, company, title)`.** URL이 없으면 `(company, title, deadline)` 폴백.
- **크롤러는 `base_crawler.py`를 상속해 `crawl(keywords, max_pages)` 인터페이스를 구현한다.**
- **Google Sheets 컬럼명은 한국어 키 사용.** 헤더 순서 변경 금지.
- **`matcher/`, `notifier/` 디렉토리는 삭제하지 말 것.**
